...
```{r libraries}
  ...
  # library(lsmeans) #cld function 
  #library(jtools) # Plot models
...
```{r fig.width=8}
legend <- get_legend(S_H_T1) 
Figure3 <- ggarrange(ggarrange(S_H_T1 + 
                                 theme(legend.position = "none",
                                      axis.title.x=element_blank()),
                              Dprop_T1 + theme(legend.position = "none", 
                                      axis.title.x=element_blank()),
                     nrow = 1, ncol = 2),
                     legend, nrow = 2, heights = c(6,1))
Figure3
#ggsave("Outputs/Fig_3.svg", Figure3, width=7, height=4,dpi = 300)
```
